...
OBX|25|ST|8004^PLCR^ACC||12.6|%|11.0-45.0||||F";
    ...
    $availableDevices = $processor->getAvailableDevices();
    echo "Available devices: " . implode(', ', $availableDevices) . "\n";
    
    if (in_array('ACON', $availableDevices)) {
        echo "✅ ACON device handler is available\n";
    } else {
        echo "❌ ACON device handler is not available\n";
        exit(1);
    }
    
    echo "\n2. Testing ACON Handler Directly\n";
    echo "--------------------------------\n";
    
    // Test ACON handler directly
    $aconHandler = new ACONHandler();
    $sysmexInserter = new SysmexCbcInserter();
    
    // Parse message manually
    $cleanData = preg_replace('/\s+/', '', $hl7Message);
    $messageData = substr($cleanData, strpos($cleanData, 'MSH'));
    
    $msg = new \Aranyasen\HL7\Message($messageData);
    ...
    if ($device === 'ACON') {
        ...
        $insertResult = $sysmexInserter->insertCbcData($cbcResults, 1, $patientInfo);
    ...
    echo "\n4. Testing Full HL7 Processing\n";
    echo "-----------------------------\n";
    
    // Create a mock connection
    $mockConnection = new class {
        public function write($data) {
            echo "Mock connection received: " . substr($data, 0, 100) . "...\n";
        }
    };
    
    // Process the message through the full processor
    $processor->processMessage($hl7Message, $mockConnection);
    
    echo "✅ Full HL7 processing completed\n";
    
    echo "\n✅ All tests completed successfully!\n";
    echo "\nSummary:\n";
    echo "- Device detection: Working\n";
    echo "- ACON handler: Working\n";
    echo "- CBC extraction: Working\n";
    echo "- Sysmex insertion: Working\n";
    echo "- Full HL7 processing: Working\n";
