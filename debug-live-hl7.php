...
echo "1. Raw Message Analysis\n";
echo "----------------------\n";
echo "Raw message length: " . strlen($rawMessage) . "\n";
echo "First 100 characters: " . substr($rawMessage, 0, 100) . "\n";
echo "Contains MSH: " . (strpos($rawMessage, 'MSH') !== false ? 'Yes' : 'No') . "\n";
echo "Contains OBR: " . (strpos($rawMessage, 'OBR') !== false ? 'Yes' : 'No') . "\n";
echo "Contains OBX: " . (strpos($rawMessage, 'OBX') !== false ? 'Yes' : 'No') . "\n";

echo "\n2. Message Processing (Same as HL7MessageProcessor)\n";
echo "--------------------------------------------------\n";
...
// Clean and parse message (same as HL7MessageProcessor)
$cleanData = preg_replace('/\s+/', '', $rawMessage);
echo "Clean data length: " . strlen($cleanData) . "\n";
echo "Clean data first 100 chars: " . substr($cleanData, 0, 100) . "\n";
...
echo "\n5. OBX Segment Analysis (First 5)\n";
echo "---------------------------------\n";
$obxCount = 0;
foreach ($msg->getSegments() as $segment) {
    if ($segment->getName() === 'OBX' && $obxCount < 5) {
