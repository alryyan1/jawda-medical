...
class PatientMedicalHistoryController extends Controller
{
     ...
     *
     * Resolved across every visit of this patient's File, not just this
     * visit's Patient row — each visit creates a new Patient row, so a
     * lookup scoped to patient_id alone would never see history recorded
     * on an earlier visit and would appear to reset every time.
     */
     ...
     *
     * Updates the same file-wide record resolved in show() above, so an
     * edit made against this visit's Patient row lands on the record other
     * visits of the same File will also see, instead of forking a new
     * per-visit copy.
     */
    public function update(Request $request, Patient $patient): JsonResponse
    {
        ...
        $existing = PatientMedicalHistory::whereIn('patient_id', $patient->siblingPatientIds())
            ->latest('updated_at')
            ->first();
        ...
        if ($existing) {
            ...
            $history = PatientMedicalHistory::create(['patient_id' => $patient->id, ...$validated]);
        }
