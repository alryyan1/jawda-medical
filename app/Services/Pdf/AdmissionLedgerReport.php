...
class AdmissionLedgerReport
{
    ...
    {
        $this->admission = $admission->load(['patient', 'ward', 'room', 'bed']);
    ...
    private function renderAdmissionInfo(): void
    {
        ...
        $admissionDateTime = $this->admission->admission_date ? Carbon::parse($this->admission->admission_date) : null;
        $admissionDate = $admissionDateTime ? $admissionDateTime->format('Y/m/d') : 'غير محدد';
        $admissionTime = $admissionDateTime ? $admissionDateTime->format('H:i') : '';
        ...
        if ($this->admission->discharge_date) {
            $dischargeDateTime = Carbon::parse($this->admission->discharge_date);
            $dischargeDate = $dischargeDateTime->format('Y/m/d');
            $dischargeTime = $dischargeDateTime->format('H:i');
