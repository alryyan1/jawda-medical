...
echo "1. Message Processing (Fixed Version)\n";
echo "------------------------------------\n";

// Clean and parse message (FIXED VERSION)
$cleanData = preg_replace('/\r\n|\r|\n/', "\r", $rawMessage);
$cleanData = preg_replace('/[ \t]+/', ' ', $cleanData);
...
foreach ($segments as $segment) {
    if ($segment->getName() === 'OBR') $obrCount++;
    if ($segment->getName() === 'OBX') $obxCount++;
}
echo "OBR segments: {$obrCount}\n";
echo "OBX segments: {$obxCount}\n";

echo "\n3. ACONHandler Testing\n";
echo "---------------------\n";
...
echo "\n4. Sysmex Insertion Test\n";
echo "------------------------\n";

if ($doctorVisitId && is_numeric($doctorVisitId) && count($cbcResults) > 0) {
    ...
    $insertResult = $sysmexInserter->insertCbcData($cbcResults, (int)$doctorVisitId, $patientInfo);
    
    if ($insertResult['success']) {
        echo "✅ Sysmex insertion successful!\n";
    ...
    echo "❌ Cannot test Sysmex insertion - missing data\n";
    echo "   Doctor Visit ID: '{$doctorVisitId}'\n";
    echo "   CBC Results Count: " . count($cbcResults) . "\n";
}
...
echo "\nSummary:\n";
echo "- Message parsing: " . (count($segments) > 0 ? "✅ Working" : "❌ Failed") . "\n";
echo "- Device detection: " . ($device === 'ACON' ? "✅ Working" : "❌ Failed") . "\n";
echo "- Doctor visit ID: " . ($doctorVisitId === '5676' ? "✅ Working" : "❌ Failed") . "\n";
echo "- CBC extraction: " . (count($cbcResults) > 0 ? "✅ Working" : "❌ Failed") . "\n";
echo "- Sysmex insertion: " . (isset($insertResult) && $insertResult['success'] ? "✅ Working" : "❌ Failed") . "\n";
